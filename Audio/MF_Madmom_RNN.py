...
from madmom.features.tempo import  DBNTempoHistogramProcessor, ACFTempoHistogramProcessor,TempoEstimationProcessor
...
class AudioFeaturesRNN(object):
    ...
    def show_features(self,beat,downbeat,tempo, text = None, waveshow = False, spectrogram = False, bar=False, track= None, label = False,image_dir = 'image/'):
        ...
        if waveshow == True : 
            ...
            plt.title(f'Audio waveform with beat and downbeat predictions {text[1]} BeatProcessor', fontsize=15)
            ...
            plt.xlim(30,60);
            plt.show()
            plt.savefig(f'{image_dir}/waveform_{text[0]}.png')
...
if __name__ == "__main__":
    ...
    feature = AudioFeaturesRNN(audio, sample_rate=sr)

    beat, downbeat, tempo = feature.madmom_features(fps = 100)
